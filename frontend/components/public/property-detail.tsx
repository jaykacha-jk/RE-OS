...
export function PublicPropertyDetail({
  ...
  const contactPhone = contact.phone ?? contact.whatsapp ?? null;
  const contactEmail = contact.email ?? null;
  const contactAddress = contact.address ?? null;
  const whatsappUrl = whatsappHref(contact.whatsapp ?? contact.phone, whatsappText);
      ...
      <div className="mt-8 grid gap-8 lg:grid-cols-[minmax(0,1fr)_360px]">
        ...
        <aside className="space-y-4 lg:sticky lg:top-24 lg:self-start">
          <div className="card p-5">
            <p className="text-xs font-bold uppercase tracking-[0.16em] text-slate-500">Contact team</p>
            <div className="mt-3 flex items-center gap-3">
              ...
              <div>
                <p className="font-bold text-slate-950">{siteName}</p>
                <p className="text-sm text-slate-500">
                  {contactPhone ? contactPhone : 'Responds within 2 hours'}
                </p>
              </div>
            </div>
            ...
            {contactEmail ? (
              <p className="mt-2 text-sm text-slate-600">
                <a href={`mailto:${contactEmail}`} className="font-medium text-teal-800 hover:underline">
                  {contactEmail}
                </a>
              </p>
            ) : null}
            {contactAddress ? <p className="mt-2 text-sm text-slate-600">{contactAddress}</p> : null}
          </div>
